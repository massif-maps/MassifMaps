...
## Example screenshots — compose them, do not just capture them

The gallery and the website both show a **wide vignette in a grid**, generated by
`scripts/capture-examples.py` into `docs/examples/screenshots/`. The rules, all learned the hard
way (see [`docs/contributing/examples.md`](../docs/contributing/examples.md) for the workflow):
...
- **Landscape, no chrome.** The script turns the device and passes `--es ui false`. A picture of
  the map, not of the app around it.
- **Pick the place and the camera deliberately.** A default camera over an arbitrary city is what
  makes a gallery look unfinished. For 3D terrain that means a real mountain, satellite imagery on
  the mesh AND roads/summit labels over it — show the capability, not just the pixels.
- **READ BACK the camera.** `ExampleActivity` logs `camera lon=… lat=…` on every start. A dark
  frame that read as "the shadows are broken" was the open Atlantic. Verify before believing.
- **Iterate with intent extras, not rebuilds** — `--es lat/lon/zoom/tilt/rotation` on
  `.ExampleActivity`. Put the numbers back into the example's `moveTo` once it looks right.
- Terrain traps: a high tilt buries a peak in the ridge behind it, and a close zoom puts the
  camera *inside* the slope (the terrain keeps a clearance above the ground).
