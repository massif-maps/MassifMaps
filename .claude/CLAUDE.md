...
- **Documentation ships in the same commit** — the [document](skills/document/SKILL.md) skill.
- **Tests ship in the same commit too** — the [test](skills/test/SKILL.md) skill.
- **Every new feature reaches the facade API** — see [below](#the-facade-api--every-new-feature-reaches-it).
...
A gallery example is ONE id on Android, iOS **and** NativeScript, plus two generators. Adding it to
one platform is a half-change. Use the [add-example](skills/add-example/SKILL.md) skill — it carries
the file layout, each platform's host API, the spec traps, the caching rule for remote sources and
the screenshot rules.

## Tests — every change ships them

`cd tests && ./run.sh` — under a second, and the only gate here that checks behaviour rather than
syntax. New work ships its own tests, in the SAME commit. How, and what belongs in the reduced
link: the [test](skills/test/SKILL.md) skill.

## Documentation — every change updates it

A change that alters behaviour, adds an option or a uniform, or invalidates something written in
`docs/` ships the doc edit in the **SAME commit** — never as a follow-up. Which home, what the bar
is, and the mechanics that fail the site build: the [document](skills/document/SKILL.md) skill.
...
| NativeScript plugin + demo                                                   | `integrations/nativescript/` | `master` | its own remote (`git -C integrations/nativescript remote -v`) |
...
- `integrations/nativescript` is a nested repo too, and it starts on `master` - branch it before editing, same as the others.
...
- **Run and extend the host tests** — `cd tests && ./run.sh`, seconds, and the only gate that checks
  behaviour rather than syntax. New work ships its own; see the [test](skills/test/SKILL.md) skill.
