...
* Map is being moved or zoomed (`onMapMoved`), with a `MapMoveReason` saying whether it was the
  user, an animation or an API call
...
public class MyMapListener : MapEventListener
  {
      ...
      public override void OnMapMoved(MapMoveReason reason)
      {
...
public class MyMapEventListener : MSFMapEventListener {
    ...
    public override func onMapMoved(_ reason: MSFMapMoveReason) {
...
class MyMapEventListener(var mapView: MapView?, var vectorDataSource: LocalVectorDataSource?) : MapEventListener() {
    ...
    override fun onMapMoved(reason: Int) {
