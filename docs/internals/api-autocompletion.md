...
## Regenerating, and shipping them
...
The C++ property table is generated by CMake on every build and therefore cannot drift. **These four
are not**, and they did: a class added to `all/modules` reached the table and the schema on the next
build, and reached the TypeScript typing, the Objective-C constants and the C enum ids only when
