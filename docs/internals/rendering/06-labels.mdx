...
What it costs: a full cycle now takes several hundred ms, so labels settle behind a fast camera
rather than with it (they keep their previous visibility meanwhile, so nothing flickers), and the
placement SELECTION shifts, since an early slice claims grid slots before a later, higher-priority
one. mapbox has the same property.
