...
The pacing is the other half of the guarantee. mapbox gets it from the frame; `VTLabelPlacementWorker`
has no frame, so an unfinished cycle would resume as fast as the CPU allows. Budget `B` with resume
delay `D` costs about `1000B/(B+D)` ms a second — 2 and 25 give 74, and measured worst cases are
98 ms/s at tilt 30 and 75 at tilt 80 against a 100 ms objective
([performance log 28](../performance-log.md)).
