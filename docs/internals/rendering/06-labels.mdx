...
`text-wrap-width` is in **style pixels**, and is the one length in `TextSymbolizer::getFormatterOptions`
that is NOT multiplied by `fontScale`. `TextFormatter::splitLines` accumulates a word's width from
`glyph.advance * _fontSize`, where `_fontSize` is the style size — so a threshold in device pixels
was compared against a width in style pixels and every label wrapped `fontScale` times too late. On
a 2.6x screen a 8-em maximum became a 21-em one, which is to say nothing wrapped at all: MapTiler's
"Médiathèque Annecy - Bonlieu" stayed on one line where the same style wraps it to two in a browser.

Where a label breaks is a property of the style, not of the display, so the offsets beside it
(`dx`/`dy`, the secondary run's) scale and this one does not.
...
changes glyph atlas mid-list. It runs **bottom to top** — what is drawn last is on top — which is the
OPPOSITE of the culler's order, where the most important label is placed FIRST so it claims its slot
(`LabelCuller::process`). The draw list used to copy the culler's direction, so of two labels that
overlap the LESS important one was drawn over the other: a road name covering the town name it
crossed. The `layerIndex` tie-break had always run bottom-to-top, which is what gave it away. Grouping the labels by atlas first — which is what a
...
- **How far the run sits off the line comes from the CR pseudo-glyph**, like it does for a point
  label: `TextFormatter::layoutLines` puts the block's vertical alignment and its `text-dy` in that
  glyph's `advance(1)`, and `buildLineVertexData` applies it perpendicular to the run. Ignoring it —
  which it did until 2026-08 — laid every line label on its **baseline**, so the text floated above
  its road by about an ascender and `text-dy` did nothing at all on a line. The culler saw the
  centred box all along: its box comes from the point pen walk (`walkGlyphs`), which never dropped it.
...
**A line label's group is its TEXT**, so the distance thins repeats of one name and leaves a
different name alone; only a point label shares one group with every other. `shield-min-distance`
used to fall in the second case whatever its placement, which made it a distance between DIFFERENT
roads: a style setting it to its own `shield-spacing` — what `mapbox2css` writes, so that a road cut
into many ways is not shielded on each — lost most of its shields instead.
...
**The run never leaves the line.** `pointAt` clamps at both ends and the fit test is exact —
`runLength > total` is `NO_ROOM`, and the pen start is clamped to `[0, total - runLength]`. This is
tangram's rule (`LineSampler::advance` clamps, `CurvedLabel::updateScreenTransform` rejects
`start < 0 || start + width > sumLength()`). Until 2026-08 both ends were given `PLACEMENT_ROOM_FACTOR`
of slack — a run up to 1.5x the line was accepted, and `pointAt` continued the line straight past
its last vertex to carry it. The slack was there to stop a run at the edge of what fits blinking as
the projected line changes length under the camera; the cost was a street name running off the end of
its road, and off the curve on a bent one, so what absorbs the flapping is now
`LINE_LAYOUT_FAILURE_GRACE` alone. The per-glyph direction window slides back to stay on the line
rather than being clamped against its end, where it would collapse and read as `UNREADABLE`.
Covered by `tests/vt/LineLabelTest.cpp`.
...
`renderLabelBatch` uses, and its halo reaches it in screen pixels like a label's. The
...
#### How much of the divide is given back: `labelPerspectiveScaling`

Cancelling the divide *entirely* is not what the reference renderers do. maplibre and mapbox damp
it: `symbol_sdf.vertex.glsl` multiplies the glyph size by
`clamp(0.5 + 0.5 * distance_ratio, 0.0, 4.0)`, so a label past the focus point does shrink — at half
the rate the projection alone would shrink it. `VectorTileLayer::setLabelPerspectiveScaling` is that
blend, `0` to `1`, reaching vt as `ViewState::labelPerspectiveScaling`:

| value | factor | what a label at 3× the focus depth looks like |
|---|---|---|
| `0` | `depth / focusDepth` | full size — a constant on-screen size, what this SDK did before 6.1 |
| `0.5` (default) | `0.5 + 0.5 · depth / focusDepth` | ⅔ size — maplibre's own damping |
| `1` | `1` | ⅓ size — fixed to the map, shrinking as fast as the geometry |

Two carve-outs. The damping applies only **past** the focus point (`factor > 1`); nearer than it the
same blend scales a label *up*, and a name lunging at the camera across the near half of a tilted
map is worse than the distant shrink is good — maplibre has this and we do not want it. And a
**callout** is exempt whatever the setting: it is a screen object whose whole point is to stay
readable at any range (see below).

This is not only a size. A `line` label must find `textLength × factor` of **world line** to sit on
(`findClippedLinePlacement`), so at `0` a road at 3× the focus depth was asked for 3× the road and
its name was dropped. Measured at Grenoble z15.57 tilt 59 in the style preview, massif-local drew
**6 road names at `0` and 9 at `0.5`**, against maplibre's ~12 — so the damping recovers part of the
gap, not all of it. What is left is architectural: maplibre decides whether a name fits its line
once at **tile build time**, in tile space, with no camera in the question at all.

`tools/style-preview` forwards `?labelperspective=` to the massif row, which is how that A/B is run.
...
- **The sides are precomputed, the choice is per pass.** `TileLayerBuilder` measures the text once
  ...
  left side). A style with neither `shield-anchors` nor `shield-text-optional` builds no variants
...
- **`shield-text-radial-offset` places it mapbox's way instead**, which is what a converted mapbox
  style wants: the distance runs from the **anchor** to the near edge of the text, on the side's own
  axis and **zero across it**, a corner taking it on the diagonal (`evaluateVariableOffset`,
  `variable_text_anchor.ts`). Read as `dx` instead, the same style came out with the icon's
  half-width added to every side gap, and with a name centred under its icon sitting one `dx` to the
  right of it. The model lives in `LabelVariants.h` and is pinned by `tests/vt/LabelAnchorAlignTest.cpp`.
- `shield-text-optional` appends a last variant that draws the icon alone. That is mapbox's
  `text-optional`; here it costs one more variant, not a second label. It does **not** need
  `shield-anchors` — a list of one placement plus the icon-only fallback is a valid list, and it is
  the common case: no MapTiler style states a variable anchor, but 25 layers of outdoor-v4 state
  `text-optional`. `buildLabelVariants` used to return early on an empty anchor list and never
  reached the fallback, so those layers drew nothing where MapTiler drops the name and keeps the pin.
...
- **…and never while the last pass is still fading** (`VTLabelPlacementWorker::MIN_PLACEMENT_INTERVAL`),
  which is maplibre's `Placement.stillRecent`: it will not begin a placement before
  `commitTime + fadeDuration`. Every tile that arrives asks for a pass, so a pan at tilt 80 ran
  several a second while a label's fade takes a whole second at the default blending speed — the
  label never reached full opacity before the next pass could drop it. Measured on the Grenoble
  preview at z16/tilt 80: 10–14 labels flipped on and off per pass, 4–13 of them mid-screen, none of
  them losing a placement (`offInvalid=0`) — they lost a collision to a label that had just arrived.

  A/B over the same six-step pan, timing every pass:

  | | passes / pan step | shortest gap | median gap | gaps &lt; 200 ms |
  |---|---|---|---|---|
  | interval 0 | 10.7 | 0 ms | 73 ms | 48 of 62 |
  | interval 1000 | 4.5 | 999 ms | 1005 ms | **0 of 25** |

  `_lastPassTime` is stamped when a pass **starts**, under the lock that claims it. Stamped when it
  ended instead, a `schedule()` arriving while the pass ran read the previous pass's time, found the
  interval already spent, and fired again the moment it finished — 202 ms between passes, measured.

  The interval and the fade are ONE duration: 300 ms, which is maplibre's `fadeDuration` and now
  `TileRenderer`'s default label blending speed (1/0.3 per second). A second of fade was long enough
  that a name was still arriving when it had reached the middle of the screen.

### A label is placed BEFORE it reaches the screen

maplibre pads the collision viewport by `viewportPadding = 100` px (`collision_index.ts`) and sizes
its grid `width + 2 × padding`, so a symbol is placed while it is still outside. Its own comment: the
padding exists "so that the bulk of the changes occur offscreen… increases label stability, but it's
expensive". A symbol placed there gets `JointPlacement::skipFade`, and `OpacityState` starts it at
opacity **1** instead of 0 — "if a subsequent viewport change brings them into view, they'll be fully
visible right away".

Both halves are ported. `ViewState::labelFrustum` is the frustum grown by `labelPadding` pixels, and
it is what `Label::updatePlacement` places against — the frustum tests AND the clip planes the point
and line searches use. Missing that last part is the whole difference: the padded frustum admitted
the label, then `findClippedPointPlacement` clipped it against the plain one and produced nothing, so
the band measured 0 labels until the clip planes were switched too. `LabelCuller::isOffscreen` then
hands anything placed outside the true viewport full opacity, and `getGridIndex` spans the padded
viewport so those labels get their own cells instead of clamping into the border ones.

**The band shrinks with tilt, which maplibre's does not.** 100 screen pixels at the horizon are
kilometres of map and thousands of labels; at top-down they are 100 pixels of map. `labelPadding` is
therefore `100 × sin(tilt)`, floored at 20 — tilt 90 is straight down here, so `sin(tilt)` IS the
foreshortening of the ground plane, and the band stays roughly constant in WORLD terms. Measured
live: `padding=100.0` top-down, `padding=50.0` at pitch 60, and 1–4 labels per pass taking the
skip-fade path.

**The band is worth nothing without the tiles that fill it.** A label cannot be placed early if it
does not exist, and it does not exist until its tile reaches the renderer. `TileLayer` sorted every
leaf tile into two buckets — inside the plain frustum it is a *visible* tile, otherwise a
*preloading* one, and preloading is **off by default** — so the first Label for the ground scrolling
in was built the frame its tile touched the viewport edge, on screen, at opacity 0.

There is now a third bucket between them: outside the viewport but inside
`ViewState::getLabelFrustum()`, the same band the culler places against, built from the same
`vt::ViewState::paddedProjectionMatrix`. Those tiles are fetched and handed over whatever
`isPreloading()` says.

**They are handed over for their labels only.** `GLTileRenderer::setVisibleTiles` takes a second
map: it joins `labelTiles`, so `buildLabelMaps` sees it, and it never reaches `tileIds` — no tile
surface, no render tile. `TileRenderer::refreshTiles` splits the draw datas on `isPreloadingTile()`,
which is exactly "the visTile was outside the view frustum", so the preloading ring stops being
drawn too. Skipping that split cost **4.3% of the frame rate** (below).

Measured on the style preview (Grenoble, z16, top-down, 381×444 pane), counting every label that
becomes visible *on screen* while still undrawn, bucketed by what it was on the previous pass:

| | without the band tiles | with them |
|---|---|---|
| passes with an on-screen entry | 85 | **0** |
| entries: never existed / had no placement / lost a collision | 165 / 81 / 33 | 0 / 0 / 0 |
| pan distance | 600 px | 230 px |

Normalised, ~46 undrawn entries per 100 px of pan against 0. The pan was very slow (the harness
throttles timers), which is the condition that most favours the baseline: tiles had unlimited time
to arrive and it still lost.

**What the band costs, in tiles.** Counted at rest, same pane, camera jumped rather than panned:

| camera (SDK tilt) | visible | label | preloading |
|---|---|---|---|
| z16, tilt 90, centred on a tile | 4 | 0 | 0 |
| z16, tilt 90, straddling two | 2 | 0 | 4 |
| z16, tilt 30, straddling | 12 | **1** | 5 |
| z13.5, tilt 30, straddling | 16 | 0 | 6 |
| z13.5, tilt 90, straddling | 2 | 0 | 2 |

**At rest the band costs 0–1 tiles.** It fills only while the viewport crosses a tile edge, and
during the pan above it held 2–3 against 2–4 visible. Those are tiles the pan is about to need
anyway, so the fetch is pulled *forward* rather than added; the only wasted work is a camera that
enters the band and turns back. The upper bound is geometric: 100 px on each side of a ~888 px
viewport is ~1.2× the visible ground at top-down, and less when tilted, since `labelPadding` scales
with `sin(tilt)` (tilt 30 above: padding 50, one extra tile on twelve).

**What it costs on the device, and why the labels-only split is the whole point.** Crosscall
`1cba1468`, `-PnativeConfig=Release -PabiFilters=arm64-v8a -PprofileRender`, default bench camera
(Grenoble z16.22 tilt 26), 8 swipes, three interleaved pairs from prebuilt APKs, idle windows
(>1600 ms) dropped. Two series, each against its own baseline — the device drifts between series, so
only within-series pairs mean anything.

| | band tiles drawn | band tiles labels-only |
|---|---|---|
| fps vs its baseline | 18.7 vs **19.5** (−4.3%, 3/3 same sign) | 18.9 vs **18.8** (+0.4%, mixed sign) |
| `renderTiles` /s | **+14.9%** | −3.2% |
| `geomDraws` /s | +2.6% | −10.3% |
| `bakes` /s | +3.0% | −16.9% |
| `live` labels | +61% | +88% |
| `labelMapsMs` /s | +37% | +24% (2.8 → 3.5) |
| `cullMs` /s | +5% | +3% (2.4 → 2.5) |

**The labels are nearly free; the drawing was the cost.** Twice as many live labels move `cullMs` by
0.1 ms/s and `labelMapsMs` by 0.7 ms/s against a 53 ms frame. Drawing the band cost 15% more render
tiles that are, by construction, off screen — which showed up as `drape` +1.7 ms, `prelude` +0.5,
`cover` +0.4 per frame. Routing them to `buildLabelMaps` alone takes the whole regression back, and
`renderTiles` ends up slightly *below* baseline because the preloading ring stopped being drawn with
them.
...
### The icon run has its own antialias ramp

The icon run keeps a fixed PIXEL size of its own — `Label::calculateVertexData` scales it by
`iconRefSize / size` and then by `iconScaleFunc`, so it does not ride the text's size ramp. The
**antialias ramp has to follow it**: `labelVsh` reads `uSDFRamp / uWidthTable[styleIndex]`, so an
icon sharing the text's style slot is smoothed as if it were the text's size.

`renderLabelPass` therefore gives the icon run a slot of its own whenever that ramp would differ,
not only when the icon has a colour, and puts the icon's own em size in the width table. Sharing the
text's, a 20 px POI icon was smoothed with a 13 px ramp — half again too wide — and every thin
stroke fattened until it closed: a fork's tines fused into one blob.

The remedy is only half of it. What the ramp cannot invent is resolution: a notch one texel wide
never reaches full coverage and closes at the size the icon is drawn. `mapbox2css` takes the densest
sprite sheet the provider serves (`@4x` for MapBox) for the same reason.
...
- A plate is **nine-sliced from one atlas cell** (`LabelPlateBitmap.h`, cached by radius and border
  width, the glyph map dedupes by pointer): four corner cells keep the radius, the edges stretch
  along one axis and the centre fills — which is what keeps the corners round at any name width.
- **Fill and border come out of that one cell, in one quad**: the cell's `r` channel is the fill's
  coverage and its `a` the whole plate's, so the ring between them is the border, and `labelFsh`
  (draw mode `PLATE`) mixes the two colours *before* the single blend —
  `vColor * r + vBorderColor * (a - r)`. The border's colour is the batch slot right after the
  fill's (`LabelPlateIndices`), which is why a reused pair has to be consecutive.
  Drawing the border as a **second quad behind the fill** is what this replaced: both quads carry
  the label's own alpha, so fill-over-border leaves `border * a * (1 - a)` showing through wherever
  `alpha < 1` — a yellow shield with a black border went dark for the length of its fade-in, and a
  translucent `text-background-opacity` stayed dark for good. Punching the fill's shape out of the
  border cell (the ring) fixes the middle of the plate but not the seam where the two meet: any
  overlap darkens, and ending the ring exactly on the fill's edge leaves an antialiasing hole
  instead. One quad is the only form with neither.
- The cell is drawn at **4 texels per style pixel** (`PLATE_SUPERSAMPLE`) and the quad is snapped to
  that same grid (`PlateCell::radius/borderWidth`, carried on the plate). A border thinner than a
  quarter pixel keeps a texel rather than rounding to none — rounded away it left a cell with no
  ring at all, i.e. a filled plate behind the fill, which is the darkening above.
...
- Each colour is one slot in the label batch, like the halo (`LabelBatchParameters::MAX_PARAMETERS`
  ...
  The quad count halved with the single-quad form: a bordered plate is one nine-slice, not two.
...
(≈60/s either way). It is proportional to the plated labels on screen: each plate is a handful of
quads built in the same loop as the glyph quads, so a screen of road shields — tens, not thousands —
is far below what the frame-time noise on this emulator can even resolve. (Measured when a plate was
three quads and a border was a second plate; it is a nine-slice and one quad now, so read it as an
order of magnitude, not a current number.)
...
### ...and a cut that is always on, in camera-relative units

`text-max-distance` is opt-in, so a style that does not set it caps nothing — measured at Paris
z16.5, tilting from 80 to 30 took live labels from 950 to 5001 and the culler from 59 to 668 ms a
second ([performance log 27](../performance-log.md)). Both GL-JS renderers cut unconditionally
instead, and not in metres:

| | constant | cuts past |
|---|---|---|
| mapbox `minPerspectiveRatio` | 0.55 | 10 × camera-to-centre |
| maplibre `perspectiveRatioCutoff` | 0.6 | 5 × camera-to-centre |

`vt::LabelDistance` ports maplibre's, the tighter of the two, as
`ratio = 0.5 + 0.5 · (cameraToCentre / distance)`. **The unit is the point.** Camera-to-centre
shrinks with zoom and grows with pitch, so one constant holds everywhere; a metre cap cuts a city
block at z18 and nothing at all at z10. Measured: at tilt 80 it cuts **nothing** (the view never
reaches 5× the centre distance looking down, so it costs nothing and removes nothing), at tilt 30 it
drops **29%** of labels before they cost a placement.

It runs *before* `updatePlacement` and the variant envelopes on purpose — those are the per-label
cost, and the horizon band is where most of the labels are. It bounds the cost of placing labels,
**not the number of them**: `labelsLive` is unchanged, because the labels are still built by
`buildLabelMaps` upstream. Tangram is the one that attacks the count, by colliding each tile against
itself on the tile worker at style-zoom + 2 and marking the losers `State::dead` for good
(`LabelCollider::process`, plus a hard 4096-label cap per tile) — neither GL-JS renderer has that.

### The pass itself is rationed

The cut bounds what each label costs; the budget bounds what a pass costs. Splitting
`LabelCuller::process` shows where to cut: **collect** (`updatePlacement` + the variant envelopes)
is ~90% of it at ~14.6 us a label, the sort ~2%, the grid insertion the rest. Collect runs *before*
the sort, so stopping it short costs no ordering among the labels that were collected.

`beginSlice(ms)` opens a wall-clock budget shared by every `process` call in a pass; each layer
keeps a cursor (`GLTileRenderer::_labelCullCursor`) and resumes there next pass. This is mapbox's
`PauseablePlacement` — 2 ms, three cursors, nothing published until the cycle commits. Two things
have to hold across a cycle, and both bit in the writing:

- **the collision grid persists**, or the second half of the labels reuse slots the first half took;
- **the view is frozen**, or the second half is collided against a grid built for another screen.

The pacing is the other half of the guarantee, and it is a **duty-cycle gate** rather than a fixed
delay: after a pass costing `C` ms, no pass may start for `C · (1000/TARGET − 1)` ms. That holds
placement to `TARGET` (90) ms of every second whatever the tilt, the cycle size, or how often the
camera asks — a cap, not a quota, so a still map spends none of it. A fixed delay was tried first
and does not work: it bounds one cycle, and once placement got cheap enough to run whole it simply
ran more often, back to 120 ms/s ([performance log 29](../performance-log.md)). Measured worst cases
are 62 ms/s at tilt 30 and 57 at tilt 80, against a 100 ms objective.

The cut is applied in TWO places, and the second is the one that matters. In the culler it can only
HIDE a label, and it reads the label's placement - which an unplaced label does not have, so the
~9000 unplaced labels a second bypassed it. In `Label::updatePlacement`, next to the existing
frustum reject and measured off the geometry bbox, it stops the placement SEARCH instead: 12 295
searches a second became ~250 ([performance log 29](../performance-log.md)). A frustum does not
bound distance; pitched at the horizon it reaches kilometres.

**Rationing is only worth it when there is something to ration.** Slicing a cheap cycle costs more
than it saves — each slice re-sorts and re-inserts its own subset, and the pacing stretches it over
many passes; applied unconditionally it took the culler from 59 to 75 ms/s at high tilt. Both
references have a bypass (mapbox `isFullPlacementRequested` / `fadeDuration == 0`, maplibre
`_forceFullPlacement`); ours is self-tuning rather than tilt-aware — a cycle is timed, and the next
one is rationed only if the last exceeded `FULL_PLACEMENT_MS`. Looking down, a cycle is ~10 ms and
runs whole; pitched, it is 60-100 ms and gets sliced.

What it costs while rationed: a cycle takes several hundred ms, so labels settle behind a fast
camera rather than with it (they keep their previous visibility meanwhile, so nothing flickers), and
the placement SELECTION shifts, since an early slice claims grid slots before a later,
higher-priority one. mapbox has the same property.

**Geometry gets no such cut, in any of the three.** The cutoff lives only in `symbol/collision_index.ts`
and the only thing that reads a collision index is symbol placement; fills, lines and extrusions
never go near it. Distant buildings keep drawing at any pitch. The asymmetry is the whole point —
a building shrinks with perspective and a glyph does not.
...
### Bold and italic are asked for by NAME, and only the shortfall is drawn on

`parseFontStyle` reads the weight and the slant out of the face NAME (`"DIN Pro Bold"` → 700,
`"… Medium Italic"` → 500 + italic), because Android reaches Roboto's bold and italic through
variable-font axes rather than separate files. `applyVariationAxes` sets whatever axis the face has
(`wght`, `wdth`, `ital`, `slnt`), and **whatever is left is synthesised per glyph** —
`FT_Outline_Embolden` and a shear matrix in `addFreeTypeGlyph`.

A static face that is ALREADY the style asked for has no axis to satisfy the request, so the request
survived to the synthesis step and was drawn on top of it. A style shipping real
`DINPro-Bold.woff2` and `DINPro-MediumItalic.woff2` files — which the bundled one does, matched by
`normalizeFontName` turning both `"DIN Pro Bold"` and `DINPro-Bold` into `dinprobold` — got **double
bold** and a real italic sheared a second time over its own slant. Measured on the eink style: 32-38%
more ink in a label than it should have had.

`subtractFaceStyle` takes the face's own style off the request first. The weight comes from OS/2
`usWeightClass`, not from `FT_STYLE_FLAG_BOLD`, because the flag only knows bold and a Medium file
is 500; only the difference is emboldened, and only upwards, since an outline cannot be thinned.
...
- **Terrain anchoring.** Label geometry is built flat at decode time, so a label must be re-anchored
  ...
  **This is the most expensive thing on the render thread over 3D terrain** — measured at ~750 000
  ...
  Two more, found on the day-cycle-light example at 2 fps with a city of bridges (2026-09-03):
  every sample asked `spanHeightAt` whether the vertex stands on a deck, against every span UNION
  — hundreds in a city, most of them the same chord — so one label cost milliseconds; the unions
  are now folded into a deduplicated chord list with bounds (`_spanChords`), 1.5–2.8 s of
  anchoring per twelve-second pan down to 65–140 ms. And the anchoring of a new tile set's labels
  no longer waits for the next frame: `setVisibleTiles` samples them on the cull thread with the
  renderer's lock released (`Label::sampleElevation`) and writes them back under it
  (`applyElevation`); the frame keeps only the DEM-tile re-anchors. `debug.massif.labelanchor 0`
  restores the in-frame path for an A/B — on the emulator it was 19–31 `prepare` spikes of
  130–190 ms per pan against none.
...
- **A halo is an OUTLINE, not a backdrop: the glyph's own coverage is punched out of it.** The halo
  is still its own quad, drawn before all of the label's ink (that order is what stops one glyph's
  halo covering its neighbour — tangram's `u_pass` does the same), but `labelFsh` subtracts the
  glyph's coverage from the halo's, so the two are disjoint and the ink never composites over a
  second copy of itself. With a solid halo underneath, ink-over-halo leaves `halo * a * (1 - a)`
  wherever `alpha < 1`: a yellow road shield with a dark `shield-icon-halo-fill` went dark for the
  length of its fade-in, and a translucent `text-fill` stayed dark for good. Same rule, and the same
  reason, as a plate's border above; mapbox and tangram both carry the artifact.
  Consequence worth knowing: a style that set a transparent fill and an opaque halo used to get a
  SOLID blob and now gets a hollow outline — which is what "halo" means.
  Text drawn as geometry (`text-clip`) lays its halo down as a separate pass too
  (`TileLayerBuilder`, pass 0 of `tesselateGlyph`), and `pointFsh` punches the ink out the same way.
- **Halo width is measured in antialias ramps, not in glyph texels.** `labelFsh` shifts the
  ...
  reaching the renderer in screen pixels is as wide as the style asks whatever raster the label
  landed on. It used to be converted with the glyph's RENDER size instead — harmless while every
  glyph was rastered at 27 texels, wrong the moment the raster ladder
...
- **`text-halo-radius` is a STYLE pixel, like `text-size`** — the symbolizer multiplies it by the
  pixel scale, and only `MAX_HALO_PIXELS` (where the encoded field runs out) caps it. It did not
  used to be: a back-compat factor of `HALO_RADIUS_SCALE · HALO_PIXELS_PER_UNIT` = 1.4725 handed the
  renderer device pixels, so a halo kept its absolute width as the dpi rose while its text grew, and
  shrank against its own glyphs. `text-size`, `text-dx` and `text-dy` never had the problem — they
  reach the renderer in **ems**, which scale with the label. Measured at La Clusaz on a 2.625x
  screen, `text-halo-radius: 1.2`: **1.8 px before, 3.1 px after**, against the 3.2 px MapBox draws
  for `text-halo-width: 1.2`. `text-min-distance` carried the same defect and was fixed with it.
  **This widens every existing style's halos on a hi-dpi screen** (×1.8 at 2.625, ×2.0 at 3).
...
- A layer whose `LabelRenderOrder` is not `LAST` draws its FLAT labels in the 2D pass, before the
  buffer for this frame exists, so it samples the previous frame's — one frame of lag while the
  camera moves. Its billboards follow the buildings (see below) and sample the current one.
