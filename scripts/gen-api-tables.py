...
def specKindOfClass(specs, bases):
  """
  Which kind builds each class, base chain included.

  A shared_ptr parameter names a BASE (TileDataSource); the kind that builds it is declared on the
  concrete classes below it, so the chain is walked to find one. Shared by the C++ table and the
  schema so a binding resolves a nested spec exactly as the SDK does.
  """
  kindOf = {}
  for entry in specs:
    cppClass = entry['cppClass']
...
def emitSpecs(specs, bases, headerDirs, outPath):
  ...
  """
  kindOf = specKindOfClass(specs, bases)
...
def emitTable(entries, bases, enums, outPath):
  ...
  # as a STRING; without this it went through strtoll and became 0 - a real value for most of
  # these, so the wrong setting applied silently instead of being refused.
  byName, ambiguous = {}, set()
  for members in enums.values():
    for member in members:
      previous = byName.get(member['name'])
      if previous is not None and previous != member['value']:
        # Two enums, one constant name, different values - a bare name cannot resolve it. Left
        # OUT rather than guessed: a wrong enum is what this table exists to stop.
        ambiguous.add(member['name'])
      byName[member['name']] = member['value']
  for name in sorted(ambiguous):
    print('warning: enum constant %s is ambiguous, not resolvable by name in a spec' % name)
    del byName[name]
  lines.append('\n// Every enum constant by name, sorted - see enumValueOf.\n')
  lines.append('static const EnumConstantEntry kEnumConstants[] = {\n')
...
def emitSchema(entries, bases, specs, methods, events, enums, docs, dispatch, outPath):
  ...
  schema = {
    ...
    # Which kind builds each class, so a binding handed a nested spec for an OBJECT property can
    # resolve it the way applyObjectSpecProperty does. Covers the hand-written factories too
    # (massif::Bitmap -> "bitmap"), which appear in no !spec and so in no `specs` entry.
    'kindOfClass': dict(sorted(specKindOfClass(specs, bases).items())),
    'specs': sorted([{
