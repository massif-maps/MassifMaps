 ...
 * constructor argument and a struct field all have to read "#aarrggbb" the same way.
 *
 * The bug this pins: a COLOR property took an ARGB NUMBER only, so "#ffb8c6d8" went through
 * asLong() and landed as 0 - a fully transparent colour, which on FogOptions.color renders
 * exactly like no fog at all and cost a debugging cycle to find.
 *
...
void testColorProperty() {
    ...
    TEST_CHECK(context->setProperty(handle, "color", PropertyValue::ofString("#ffb8c6d8")) == RESULT_OK &&
               argb(fog->getColor()) == 0xffb8c6d8u,
               "the same colour written as \"#aarrggbb\" lands on the same value");
    ...
    context->setProperty(handle, "color", PropertyValue::ofString("#ffzzc6d8"));
    TEST_CHECK(argb(fog->getColor()) == 0xffb8c6d8u, "a hex string with a bad digit is refused");
    context->setProperty(handle, "color", PropertyValue::ofString("#ffb8c6d"));
...
void testColorSpec() {
    ...
    spec["color"] = Variant(std::string("#ffb8c6d8"));
    ...
    TEST_CHECK(argb(colorAt(Variant(spec), "color", 0)) == 0xffb8c6d8u,
               "a spec constructor argument takes \"#aarrggbb\"");
...
void testColorDecode() {
    Color color(0);
    TEST_CHECK(StructCodec::decodeColor(Variant(std::string("#ffb8c6d8")), color) &&
               argb(color) == 0xffb8c6d8u, "decodeColor reads \"#aarrggbb\"");
    ...
    TEST_CHECK(StructCodec::decodeColor(PropertyValue::ofString("#ffb8c6d8"), color) &&
